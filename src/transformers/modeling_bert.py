...
class BertForTokenClassifications(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        config.num_labels=1
        self.num_labels = config.num_labels
    ...
    Examples::
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                loss = lossfct(active_logits.view(1,-1), active_labels.view(1,-1))
            else:
              labels=labels.float()
              loss = lossfct(logits.view(1,-1), labels.view(1,-1))
        ...
        return loss  # (loss), scores, (hidden_states), (attentions)
...
class ProductBert(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class BertForTokenClassification_product(BertPreTrainedModel):
  # def __init__(self, config):
  #       super().__init__(config)
  #       self.config = config
